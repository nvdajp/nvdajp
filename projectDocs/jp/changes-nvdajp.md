...
`source/NVDAHelper/__init__.py` の `nvdaControllerInternal_IMEOpenStatusUpdate()` 関数で、IME の開閉状態が変更されたとき（半角全角キーが押されたとき）にビープ音を鳴らす機能を実装しています。

* **目的**: 半角全角キーが押されたときに、音声メッセージではなくビープ音のみで通知する
* **実装**:
  * `nvdajpImeBeep` 設定が有効な場合、IME の開閉状態が変更されたときにビープ音を鳴らす
  * IME が開いたとき: 880Hz のビープ音（20ms）
  * IME が閉じたとき: 440Hz のビープ音（20ms）
  * ビープ音が鳴る場合は、通常のメッセージ表示（「IME opened」「IME closed」）をスキップする
