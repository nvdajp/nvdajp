...
### 追加仕様: US2級のみ `www.` 形式のドメインで2級適用を抑止（nvdajp/nvdajp#649）
...
- **US 2級（`ja-jp-comp6-us-g2.utb`）**: 外国語引用符内でも、`www.sample.com` のような `www.` で始まるスキーム無しドメインには US 2級を適用しない。
...
`eng2Harness.json`（eng2）に `www.sample.com` の回帰ケースを追加し、
