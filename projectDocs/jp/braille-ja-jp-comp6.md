...
### 追加仕様: US 2級のみ、ドット区切り英数字トークンで2級適用を抑止（nvdajp/nvdajp#649）

#### 背景

外国語引用符 `⠦...⠴` の内側に対して liblouis（UEB/US 2級）を適用すると、URL/ドメイン等の**コンピュータ表記**に対して、
英語2級のインジケータ（前置記号）や縮約が混入し、読みやすさ・逆変換性が低下するケースがある。
特に US 2級では、`www.example.com` や `v1.4` のような「ドット区切りの英数字トークン」に対して、インジケータ（前置記号）等が混入しやすいことが報告されている。

一方で translator2 には既に「情報処理点字」判定があり、`://` や `@` 等を含むトークンは `⠠⠦...⠠⠴` で囲んで
外国語引用符の対象外にする（= liblouis を適用しない）。しかし、`www.example.com` のようなケースは現状この判定に含まれず、
外国語引用符 `⠦...⠴` に入り、2級変換が適用されてしまう。

#### 方針（UEB と US で仕様を分離）

* **UEB 2級（`ja-jp-comp6-ueb-g2.utb`）**: 現状の仕様を維持する（外国語引用符内は原則として liblouis UEB 2級を適用）。UEB 2級でもドット区切りトークンに対してインジケータが混入するケース（例: `abc.123.jp` の `⠰⠚⠏` 部分）は存在するが、今回報告・再現確認ができた範囲は US 2級のみであるため、UEB 2級への対応は将来の課題とする。
* **US 2級（`ja-jp-comp6-us-g2.utb`）**: 外国語引用符内でも、ドット `.` で区切られた英数字トークン（例: `www.example.com`, `abc.123.jp`, `h26a.pdf`, `v1.4`）には US 2級を適用しない。

#### US 2級モードにおける抑止規則

US 2級モードでは、外国語引用符 `⠦...⠴` の内側であっても、次の条件を満たすトークンには
liblouis（US 2級）を適用せず**素通し**する（囲みは `⠦...⠴` のまま維持する）。
「素通し」とは、translator2 が事前に生成した grade 1 変換済みの `outbuf` の内容をそのまま使用することを指す（liblouis による上書きを行わない）。

* **ドット区切り英数字トークン（例: `www.example.com`, `abc.123.jp`, `h26a.pdf`, `v1.4`）**:
  * 空白を含まない
  * ドット `.` で区切られたラベルが 2 個以上
  * 各ラベルは先頭が英数字、以降は英数字・ハイフン・アンダースコア

`://`、`\`、`@` 等を含む情報処理点字対象のトークンは、既存どおり translator2 が `⠠⠦...⠠⠴` にするため、
この追加規則の対象外である。

#### 期待する挙動（例）

* 入力: `www.example.com`
  * **UEB 2級**: 現状どおり（UEB 2級適用の可否は既存挙動に委ねる）
  * **US 2級**: `⠦www.example.com⠴`（追加の前置記号や縮約を出さず、ドメイン表記を保持する）
    * 実際の点字セル列: `⠦⠺⠺⠺⠲⠑⠭⠁⠍⠏⠇⠑⠲⠉⠕⠍⠴`（grade 1 変換結果そのまま）

* 入力: `v1.4`
  * **UEB 2級**: 現状どおり
  * **US 2級**: `⠦v1.4⠴`（小数点が US 2級の指示符に置換されない）
    * 実際の点字セル列: `⠦⠧⠼⠁⠲⠙⠴`（grade 1 変換結果そのまま、`⠨` 指示符なし）

#### テスト要件（JP smoke）

`eng2Harness.json`（eng2）に `www.example.com` / `v1.4` 等の回帰ケースを追加し、
UEB 2級モードの既存挙動と、US 2級モードで「2級変換を抑止して保持される」ことを CI（`jpSmokeTests`）で担保する。

### 既知の失敗・スキップ規約（_input /tput /ueb_g2 /_s_g2）
