...
### 追加仕様案: US2級のみ「情報処理点字を優先」して2級適用を抑止（nvdajp/nvdajp#649）

#### 背景

外国語引用符 `⠦...⠴` の内側に対して liblouis（UEB/US 2級）を適用すると、URL/ドメイン等の**コンピュータ表記**に対して、
英語2級のインジケータ（前置記号）や縮約が混入し、読みやすさ・逆変換性が低下するケースがある。
特に US 2級では、`www.sample.com` のようなスキーム無しドメインに対して追加セルが出やすいことが報告されている。

一方で translator2 には既に「情報処理点字」判定があり、`://` や `@` 等を含むトークンは `⠠⠦...⠠⠴` で囲んで
外国語引用符の対象外にする（= liblouis を適用しない）。しかし、`www.sample.com` のようなケースは現状この判定に含まれず、
外国語引用符 `⠦...⠴` に入り、2級変換が適用されてしまう。

#### 方針（UEB と US で仕様を分離）

- **UEB 2級（`ja-jp-comp6-ueb-g2.utb`）**: 現状の仕様を維持する（外国語引用符内は原則として liblouis UEB 2級を適用）。
- **US 2級（`ja-jp-comp6-us-g2.utb`）**: 外国語引用符内でも、**情報処理点字（コンピュータ表記）に該当するトークンには US 2級を適用しない**。

#### US 2級モードにおける抑止規則（仕様案）

US 2級モードでは、外国語引用符 `⠦...⠴` の内側であっても、次のいずれかに該当するトークンは
**情報処理点字として扱い**、liblouis（US 2級）を適用せず**素通し**する（囲みは `⠦...⠴` のまま維持する）。

- **URL/パス/クエリ等**: `://`、`\`、`/`、`?`、`#`、`%` のいずれかを含む
- **メールアドレス**: `@` を含み、前後に英数字がある（既存の情報処理点字判定と整合）
- **スキーム無しドメイン（例: `www.sample.com`）**:
  - 空白を含まない
  - `.` を含む
  - 概ね英数字・ハイフン・ドット（必要に応じて `_` を許容）のみで構成される
  - 先頭/末尾が `.` や `-` でない
  - ラベルが2個以上（`a.b` 以上）
...
- 入力: `www.sample.com`
  ...
  - **US 2級**: `⠦www.sample.com⠴`（追加の前置記号や縮約を出さず、ドメイン表記を保持する）

#### テスト要件（JP smoke）

この仕様を採用する場合、`harness.json`（translator2）または `eng2Harness.json`（eng2）に `www.sample.com` の回帰ケースを追加し、
US 2級モードで「2級変換を抑止して保持される」ことを CI（`jpSmokeTests`）で担保する。
