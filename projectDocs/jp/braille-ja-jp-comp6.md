...
* **liblouis との相違**: `h26a.pdf`・`v1.4` の US 2級など、liblouis が先頭数符 ⠰ を付与するケースは、eng2Harness の期待値（文献準拠）と異なる。`_us_g2` でスキップ。
* **長文ケース**: マスあけ表現（⠥⠰ と ⠄⠰）の相違も `_output` / `_ueb_g2` / `_us_g2` でスキップ。
