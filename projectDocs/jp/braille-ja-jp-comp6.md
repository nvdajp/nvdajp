...
* `eng2Harness.json` を読み込むテストランナーを実装する
* `output`（1級）が既存エンジンの出力と一致することを CI で検証する
* `ueb_g2` / `us_g2` のテストは最初はスキップ（`_ueb_g2` 規約）

#### ステップ2: louisHelper.py からの関数渡し

* `louisHelper.py` で `louis.translate` の関数オブジェクトと選択テーブル名を `jpTranslate()` に渡す
* テーブル選択UIで「日本語6点＋UEB2級」「日本語6点＋US2級」を選べるようにする（`brailleTables.py` に登録）
* この段階では phase2 は素通し（1級のまま）

#### ステップ3: phase2 の実装

* translator2 内で外国語引用符の中身を抽出し、`louisTranslate()` で2級変換する
* 変換結果を元のテキストに埋め戻し、ポジションマッピングを調整する
* `eng2Harness.json` の `ueb_g2` / `us_g2` テストを有効化して検証する

#### ステップ4: 外国語引用符の適用ルール見直し

* 現在のエンジンは外国語引用符の適用がピリオド有無で変わる（issue #304 コメント参照）
* 2級点字との整合性を考慮してルールを再検討する
* 日本語情報処理点字との使い分けを明確にする
