...
*. **柔軟性**: 様々なVisual Studioエディションに対応
*

*. **将来の拡張性**: Visual Studio 2025など、将来のバージョンにも対応可能

*. **フォールバック**: `vswhere`が存在しない環境でも動作する
...
*. `vs_utils.py`に`find_vcvars_with_vswhere()`と`find_vcvarsall_with_vswhere()`関数を追加
*

*. `find_vcvars()`と`find_vcvarsall()`関数を修正し、`vswhere`を優先、直接パス検索をフォールバックとする
*. `scons_jp.py`のコード変更は不要（`vs_utils.py`の内部実装の変更により自動的に`vswhere`を使用）
*. `vcsetup.ps1`で`vswhere`を優先し、Python検索、直接パス検索の順でフォールバック
*
*
**影響範囲**:
...
*. ✅ `vcsetup.cmd`は`vs_utils.py`を使用しているため、自動的に`vswhere`を使用
*
**検証結果**:
