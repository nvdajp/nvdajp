...
* `prepareChrome` 直後、ページ読み込み完了時点でフォーカスが「最初のフォーカス可能要素の前」にある場合と「既に最初の要素上」にある場合で揺れることがある。
* 代表例として `test_aria_details_noVBufNoTextInterface`（ARIA details noVbuf）では、1回目の Tab で「focus in app」を期待する実装だと、既に最初のボタンにフォーカスしている環境で不安定になる。
...
### 原因4: ページ読み込みとフォーカス付与のタイミング差

* Chrome や OS のフォーカス付与のタイミングが環境・負荷で変わり、`prepareChrome` 完了直後のフォーカス位置が一意に定まらないためである。
...
### 対処4: 初期フォーカス差を許容してから同一条件でアサートする

* 1回目のキー操作の結果で「どちらの初期状態にいたか」を許容し、そのうえで目的の要素（例: aria-details 付きボタン）に至った状態で同一のアサートを行う。対処1（複数候補の許容）・対処3（目的達成で評価）の考え方で吸収する。
