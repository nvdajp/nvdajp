...
このフォルダは、日本語版に固有の運用・方針・進行中タスクを集約する着地点です。人間開発者向けの恒常情報は `readme-nvdajp.md` に、上流の一般開発情報は `projectDocs/dev/createDevEnvironment.md` を参照してください。
...
## CI/ビルド クイックスタート
- 型チェックのみ（上流寄せ・安全導入）: `.github/workflows/nvbeta-typecheck-311x86.yml`
...
## ポリシー（抜粋）
- 上流との差分は最小に保つ。差分は明示的な場所に集約する
- 可能な限り `SCons`/純 Python を優先し、`.cmd`/`7z`/`nmake` など外部依存は段階的に削減
- CI の前提（Python/ランナー/キャッシュ）は上流に準拠する方針で段階導入
