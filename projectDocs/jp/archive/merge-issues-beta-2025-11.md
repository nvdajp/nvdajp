...
> **注意**: このドキュメントは2025年11月時点の問題点まとめです。**2026.1への取り込みは完了しています（2026-01-09）**。参考用として保持しています。
>
> このドキュメントには、betajp-251206ブランチ（x64実験）からバックポートされた内容が含まれています。このブランチ（betajp-251206v4）は x86 ビルドを維持しているため、Python 3.13 x64 前提の記述はこのブランチには当てはまりません。

本ファイルは `projectDocs/jp/roadmap.md` に基づき、2025年11月時点の「本家 nvaccess/beta を日本語版へ取り込み」作業で発生していた主な問題点を一箇所に集約したものです。前提は 3.13 x64 専念・差分最小・CI は上流構成に整合しつつ JP パッチ最小です。
