...
## result_mismatch（一人→ヒトリ等）の再発防止チェックリスト
...
5. `jptools/runJpSmokeTests.ps1` の CI 分岐で、DIC_VERSION/DIC_CODEPAGE を削除してから jtalkSync を呼んでいるか（キャッシュ由来の古い辞書を回避）
6. `miscDepsJp/jptools/jtalk/custom_dic_maker.py` に 一人→ヒトリ、おはようございます→オハヨー ゴザイマス 等のエントリがあるか
...
**詳細**: `projectDocs/jp/tab-character-analysis.md`、`projectDocs/jp/archive/codepage-investigation-history.md`
