...
これらは`miscDepsJp/include/python-jtalk/vcsetup.cmd`経由で呼び出されるため、影響なし
...
powershell -ExecutionPolicy Bypass -NoProfile -File "%~dp0vcsetup.ps1" %ARCH%
exit /b %ERRORLEVEL%
```
...
PowerShellスクリプトから設定した環境変数は、呼び出し元のバッチスクリプトに継承される必要がある。
