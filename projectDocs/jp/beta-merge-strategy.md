...
4. **改良アプローチでも残る課題**
   - 既存の履歴は保持されるが、新しいブランチへの移行には大規模な作業が必要
   - 2つのブランチが共存するため、開発フローが複雑になる可能性がある
...
**Unrelated historiesという状態自体は問題ではありません**。Gitは異なる履歴を持つブランチを同じリポジトリ内に共存させることを標準的に許容します。

**実際の問題は、マージ時に大量のコンフリクトが発生すること**です。そのため、現在の状態では、**cherry-pick ベースの段階的取り込み**が現実的です。直接マージは大量のコンフリクトが発生するため、継続的な運用には不向きです。

**本家の特定タグを基点としたブランチの切り直し**（既存ブランチをアーカイブとして残す改良版を含む）は技術的に可能ですが、大規模な作業が必要なため、現時点では推奨しません。

**改良アプローチ**（既存ブランチをアーカイブとして残す）により、既存の履歴は保持されますが、新しいブランチへの移行には大規模な作業が必要です。将来的に、大規模なリファクタリングや新しいメジャーバージョンの開始時に検討する価値があります。
