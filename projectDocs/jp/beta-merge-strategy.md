...
1. **Gitの構造的制約**
   ...
   - `--allow-unrelated-histories`は「強制的にマージする」だけで、問題を解決するものではない
...
3. **歴史的な経緯**
   - 日本語版は別のリポジトリから開始された可能性がある
   - 本家（nvaccess/nvda）とは独立して進化してきた
   - 後から共通の祖先を作ることは不可能
...
Unrelated historiesは解決できませんが、**cherry-pickベースの段階的取り込み**で対処できます：
