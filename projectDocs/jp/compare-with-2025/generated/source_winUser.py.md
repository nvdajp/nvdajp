...
@@ -154,6 +192,7 @@ class GUITHREADINFO(Structure):
...
@@ -236,9 +275,15 @@ class GUITHREADINFO(Structure):
 VK_MENU = 18
 VK_PAUSE = 19
 VK_CAPITAL = 20
+# BEGIN JP PATCH
...
+# END JP PATCH
 ...
 VK_CONVERT = 0x1C
 VK_NONCONVERT = 0x1D
@@ -428,12 +473,12 @@ class MSGFLT(enum.IntEnum):
...
+	return c_short(HIWORD(lp)).value
 ...
 def MAKELONG(lo, hi):
@@ -470,111 +515,106 @@ def MAKELONG(lo, hi):
...
@@ -582,105 +622,96 @@ def mouse_event(*args):
...
+	res = _user32.FindWindow(className, windowName)
 	if res == 0:
 		raise WinError()
 	return res
@@ -695,41 +726,38 @@ def FindWindow(className, windowName):
...
@@ -739,7 +767,7 @@ class STICKYKEYS(Structure):
 	)
...
@@ -750,100 +778,34 @@ def __init__(self, **kwargs):
...
@@ -860,8 +822,15 @@ def __init__(self, hwnd, idEvent, elapse, timerFunc=None):
 		self.hwnd = hwnd
...
+		self.ident = _user32.SetTimer(hwnd, idEvent, elapse, self.timerFunc)
 		if self.ident == 0:
 			raise WinError()
 		if not hwnd:
@@ -874,7 +843,7 @@ def terminate(self):
...
@@ -888,17 +857,17 @@ def openClipboard(hwndOwner=None):
...
@@ -906,15 +875,15 @@ def getClipboardData(format):
...
@@ -928,10 +897,45 @@ def setClipboardData(format, data):
