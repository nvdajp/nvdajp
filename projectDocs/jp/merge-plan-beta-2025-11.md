...
- [ ] ビルド: `scons source --all-cores` が正常終了
- [ ] 単体テスト: `rununittests.bat` が通過（最低限）

### 推奨チェック
- [ ] Lint: `uv run ruff format --check && uv run ruff check` (advisory)
- [ ] 翻訳チェック: `ci/scripts/tests/translationCheck.ps1` (advisory)
- [ ] CI: GitHub Actions で主要ジョブがグリーン
