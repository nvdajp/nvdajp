...
* [ ] **コードページと文字コード関連の改善**
  ...
  * **前提条件**: CI環境での`test_pass2`失敗をローカルで再現できるようにする
    ...
    * **調査結果（2026-01-13）**:
      ...
      * 🔍 **重要な発見: 辞書ビルド時のコードページ設定が関係している可能性**
        * `scons_jp.py`では`chcp 932`を実行してから辞書をビルド（789行目）
        * しかし、`chcp 932 >nul 2>&1 || echo Warning: chcp 932 failed, relying on dicrc config`というコードがある
        * CI環境（コードページ1252）では`chcp 932`が失敗する可能性がある
        * その場合、辞書はコードページ1252の環境でビルドされる
        * **仮説**: 辞書ビルド時のコードページとテスト実行時のコードページが一致していない可能性
      * ⏳ **次の調査項目**:
        1. CI環境で辞書ビルド時に`chcp 932`が実行されているか（ログを確認）
        2. 辞書ビルド時のコードページとテスト実行時のコードページが一致しているか
        3. `dicrc`の`config-charset=sjis`設定が正しく機能しているか
        4. 環境変数の違い（PYTHONUTF8、UV_PYTHON_PREFERENCEなど）
        5. タイミングの問題（非同期処理、リソース読み込み順序など）
