...
* **ついでに実施可能（ローカルマトリクス整備と並行可）**: overlay処理の廃止、コード品質の改善
* **ついでに実施可能（CI統合に触れる際）**: CI基盤の更新
...
* [ ] **Phase 3: タイプライブラリの上書き廃止（独立したPRで実施）**
...
* [ ] **Phase 4: 不要な古い .cmd スクリプトの削除（独立したPRで実施）**
  * 目的: SCons/純Python化方針に合わせ、未使用の `.cmd` を整理
  * 作業: 使用実績を確認し、未使用スクリプトを削除（削除リストはPRで明示）
  * 検証: 署名ビルド/ローカルビルド/CI が従来通り通ること

* [ ] **Phase 5: overlay 処理の廃止とコピー処理の削減（x86環境でのリファクタリング）**（優先度：中）
  * **注**: ローカルマトリクス整備と並行で進められる改善項目。ビルドプロセス簡素化によりテスト安定化が楽になる。
...
* [ ] **Phase 6: コード品質の改善（x86環境で実施可能）**（優先度：中）
...
### 段階2: ローカル開発環境でのマトリクス実行整備

* [ ] **Phase 2: ローカル開発環境でのjpSmokeTest x86/x64マトリクス実行の実現（最優先）**
* **注**: Phase 1.2（DLLパス構造の統一）と Phase 1.3（libmecab.dllのソースビルド化）は完了済みを前提。
  ...
  * **段階的実装計画**:
    ...
    2. **Phase 2.2: ローカル環境でのx86/x64マトリクス実行の実現**
    ...
    4. **Phase 2.4: CI統合**
       ...
       * **補足**: x86 実行・`-SkipInstall -SkipOverlay` で安定化済み。x64 マトリクス統合は後続フェーズで検討。
  ...
  * **注意点**:
    ...
    * Phase 1.3（libmecab.dllのソースビルド化）は完了済み
    * ローカル環境でのMSVC環境の切り替えが必要（x86/x64）
    * 安定版リリースに影響を与えない範囲で実施
  * **検証方法**:
    * **Phase 2.1完了後**: 既存のx86ビルドが正常に動作することを確認（`scons source dist launcher`、jpSmokeTest成功、安定版リリースに影響なし）
    * **Phase 2.2完了後**: ローカル環境でx86/x64マトリクス実行が成功することを確認（`runJpSmokeTests.ps1 -Parallel`、両アーキテクチャで成功、ビルド成果物の分離確認）
    * **Phase 2.3完了後**: 複数回の実行で安定して成功することを確認（リソース競合なし、エラーハンドリング適切、ドキュメント整備）
    * **Phase 2.4完了後**: CIでjpSmokeTestが安定して成功することを確認（既にx86で統合済み。x64統合は後続で検討）

### 段階3: Python 3.13 x64対応（優先度：高）

**目標**: Python 3.13 x64対応を段階的に実施

* [ ] **Phase 3.1: Python 3.13対応の準備**
  * 依存関係の互換性確認
  * 型チェックの通過確認
  * 単体テストの通過確認

* [ ] **Phase 3.2: Python 3.13対応の実施**
  * 小さなPR単位で変更
  * 各PRで全テスト通過を確認
  * 問題があれば即座に修正
...
* [ ] **Phase 3.3: x64対応の実施**
  ...
  * **libmecab.dll の x64 移行（安全な段階的移行）**
    * **現状**: `miscDepsJp/source/synthDrivers/jtalk/libmecab.dll` はバイナリでリポジトリに入っている **移行方針**:
...
* [ ] **Phase 4.1: JP Braille テスト修正**
  * `jpBrailleRunner.pass2()` の問題を特定
  * ローカル環境で再現と修正
  * 修正後に全テスト通過を確認

* [ ] **Phase 4.2: System Tests タイムアウト修正**
  * スピーチ合成エンジン初期化の問題を特定
  * CI環境とローカル環境の差分を確認
  * 修正後に全テスト通過を確認

### 段階5: x86環境でのリファクタリング完了後の検証（優先度：中）

**目標**: DLLパス統一とコード品質改善の完了後に、全機能が正常に動作することを確認

* [ ] **Phase 5.1: リファクタリング後の統合テスト**
