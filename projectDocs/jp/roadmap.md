# 日本語版ロードマップ（2026-01更新、差分最小化フェーズ開始）
...
* ⏳ **タスク 3b.4: x64移行後の変更の取り込み** - 進行中（2026-01-08時点で主要コミット取り込み完了、差分最小化フェーズ開始）
* ⏳ **タスク 3b.5: 差分最小化** - 開始（`compare-with-beta`ベースでJP固有でない差分を順次適用）
...
* [ ] **タスク 3b.4: x64移行後の変更の取り込み（`58dd14767` 以降）**
  ...
  * **現在の進捗**（2026-01-08）:
    ...
    - ⏳ **差分最小化フェーズ**: `compare-with-beta`を見ながら、JP固有でない差分を順次適用
      - **現状**: 136ファイルが変更されている（JP PATCHマーカーが286箇所）
      - **方針**: JP PATCHマーカーがない差分は`nvaccess/beta`の変更を適用
      - **優先順位**: 
        1. 明らかにマージ漏れ（例: `source/core.py`の`screenCurtain`統合）
        2. `nvaccess/beta`の正しい変更（例: `source/config/registry.py`の`_deleteKeyAndSubkeys`削除）
        3. 不明な差分は個別確認
  * 参照: `projectDocs/jp/stage3b-x64-migration-plan.md`

#### 優先度：高（リリース品質に影響）

* [ ] **タスク 3b.5: 差分最小化（`compare-with-beta`ベース）**
  * **理由**: 本家版との差分を最小化しながら、順序立てて基盤整合を進めるため
  * **現状**（2026-01-08）:
    - 変更されたファイル: 136（JP PATCHマーカーが286箇所）
    - 追加されたファイル: 58（JP固有のファイル）
    - 削除されたファイル: 6
    - 同一ファイル: 381
    - 変更率: 26.31%
    - 未マージコミット: 632（翻訳関連を除くと実質は少ない）
  * **方針**:
    1. `compare-with-beta`の差分を分析
    2. JP PATCHマーカーがある差分 → JP固有、保持
    3. JP PATCHマーカーがない差分 → `nvaccess/beta`の変更を適用
    4. 優先順位:
       - 明らかにマージ漏れ（例: `source/core.py`の`screenCurtain`統合、`source/config/registry.py`の変更）
       - `nvaccess/beta`の正しい変更（リファクタリング、バグ修正）
       - 不明な差分は個別確認
  * **実施方法**:
    - 小さな単位で順次適用
