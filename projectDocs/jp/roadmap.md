...
* [x] **タスク 1.3: タイプライブラリの上書き廃止（独立したPRで実施）**
...
* [ ] **タスク 1.4: 不要な古い .cmd スクリプトの削除（独立したPRで実施）**
...
* [ ] **タスク 1.5: overlay 処理の廃止とコピー処理の削減（x86環境でのリファクタリング）**（優先度：中）
  * **注**: ローカルマトリクス整備と並行で進められる改善。ビルドプロセス簡素化によりテスト安定化が楽になる。
