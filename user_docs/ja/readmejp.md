...
「レビューカーソルの現在の文字を報告」（デスクトップ配列：テンキー2 ラップトップ配列：NVDA+ピリオド）を4回押すと、「説明モード」「読みかたモード」の切替ができます。初期設定では「説明モード」です。
...
#### 半角のピッチ変更率(-100から100) {#JpSettingsHalfShapePitchChange}
...
#### 変換候補の番号の報告 {#JpSettingsAnnounceCandidateNumber}
...
#### 日本語版の文字入力拡張 {#JpSettingsEnableKeyEvents}
...
#### テキスト編集で改行を報告 {#JpSettingsAnnounceNewLine}
...
#### 文字説明モード {#JpSettingsCharacterDescriptionMode}

チェックすると、文字をレビューするときなどに「日 日曜日の日」のように詳細に読み上げる「説明モード」になります。チェックなしにすると「日 ヒ」のように読む「読みかたモード」になります。
この設定は「レビュー内の現在の文字を報告」コマンド（デスクトップ配列：テンキー2 ラップトップ配列：NVDA+ピリオド）をすばやく4回押して切り替える状態と連動しています。
初期値はチェックされています。

**補足**：この設定をチェックなし（読みかたモード）にした状態で、NVDA の「音声」設定にある「テキストカーソル移動のすこし後で文字を説明」をチェック（有効）にする使い方もあります。こうすることで、カーソルを動かして読むときは素早くシンプルな読みが行われ、少し立ち止まったときだけ詳細な文字説明の読み上げを後から補足させることができます。
...
#### 英語2級点字の出力テーブル（外国語引用符内）

日本語点字の規則では、外国語引用符 `⠦...⠴`（2の点・3の点・6の点で始まり、3の点・5の点・6の点で終わる点字で囲まれた部分）の内側は、英語点字の表記に従って書きます。

外国語引用符の内側は、「日本語6点情報処理点字」では英語1級点字（uncontracted、縮約なし）です。

次の2つの出力テーブルを選ぶと、引用符内だけ英語2級点字（contracted、短縮形）で出力されます。

| 出力テーブル名 | 引用符内の変換 | 備考 |
|----------------|----------------|------|
| 日本語6点情報処理点字(外国語引用符内はUEB2級) | UEB 2級（Unified English Braille） | 国際的に広く使われる英語2級点字です。 |
| 日本語6点情報処理点字(外国語引用符内はUS2級) | US 2級（American English Braille） | アメリカ英語の2級点字です。数符や小数点などが UEB と異なります。 |

どちらのテーブルでも、引用符の外側は従来どおり日本語6点情報処理点字で出力されます。「コンピューター点字(NABCC)を使用」を有効にした場合も、カーソル位置の NABCC 表示はこれまで通り利用できます。

#### ロシア文字（キリル文字）の点字出力

「日本語6点情報処理点字」系の出力テーブルでは、ロシア文字（キリル文字）を国際的に使われるロシア点字の配列で出力します（[issue #224](https://github.com/nvdajp/nvdajp/issues/224)）。これは NVDA 日本語版の独自仕様で、次のように表記します。

* 小文字は6点式ロシア点字の配列をそのまま使います。例えば а は1の点、б は1・2の点です。
* 大文字は、対応する小文字と同じ配列に7の点を加えて表します。例えば А は1の点と7の点です。6点のみの点字ディスプレイでは大文字と小文字は区別されません。
* 外国語引用符などの囲み記号は使いません。日本語や英語との区別は文脈と7の点によります。

#### ギリシャ文字の点字出力

「日本語6点情報処理点字」系の出力テーブルでは、ギリシャ文字を国際的に使われるギリシャ点字の配列で出力します（[issue #456](https://github.com/nvdajp/nvdajp/issues/456)）。これは NVDA 日本語版の独自仕様で、ロシア文字と同じ方式です。

* 小文字は国際式ギリシャ点字の配列をそのまま使います（UEB のギリシャ文字と同じ基本配列で、多くは対応するラテン文字と同形です）。例えば α は1の点、π は1・2・3・4の点です。
* 大文字は、対応する小文字と同じ配列に7の点を加えて表します。例えば Α は1の点と7の点です。6点のみの点字ディスプレイでは大文字と小文字は区別されません。
* 外国語引用符などの囲み記号は使いません。
* 対象は現代ギリシャ文字の24字と語末シグマ（ς）です。ϑ・ϕ・µ（マイクロ記号）などの異体字は Unicode 正規化により対応する基本文字として出力されます。アクセント付きのギリシャ文字には対応していません。
...
* 拡大鏡の「倍率を上げる」を、JIS 配列のキーボードでは `NVDA+shift+;`（セミコロン）でも実行できるようになりました。本家版の `NVDA+shift+=`（イコール）は US 配列向けで、JIS 配列では `=` が物理キーではないためです。
* NVDA設定ダイアログ「日本語設定」カテゴリで F1 キーを押すと、該当する項目の説明（readmejp.html）を開けるようになりました。
* Microsoft Word（UIA 経路）で、二重取り消し線を一重取り消し線と区別して報告するようになりました。
...
* 日本語版独自の音声ドライバー Haruka (nvdajp) を廃止しました。
* 日本語設定「ヘルプを独自のウィンドウで開く」を廃止しました。
* 日本語設定「数式を英語で読み上げる」を廃止しました。MathCAT は日本語に対応していないため、数式は既定で英語で読み上げられます。
