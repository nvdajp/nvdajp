	...
	for cnt, line in enumerate(fp):
		...
		if line and not line.startswith("#"):
			...
			if fields and fields[0] in cldrDict:
				if fields[1] == "ハート":
					print(f"symbols.dic:{cnt + 1} {fields!r} => {cldrDict[fields[0]]}")
...
with open(characters, encoding="utf-8") as fp:
	for cnt, line in enumerate(fp):
		...
		if line and not line.startswith("#"):
			...
			if fields and fields[0] in cldrDict:
				if fields[2] == "[ハート]":
					print(f"characters.dic:{cnt + 1} {fields!r} => {cldrDict[fields[0]]}")
...
with open(characterDescriptions, encoding="utf-8") as fp:
	for cnt, line in enumerate(fp):
		...
		if line and not line.startswith("#"):
			...
			if fields and fields[0] in cldrDict:
				print(
					f"characterDescriptions.dic:{cnt + 1} {fields!r} => {cldrDict[fields[0]]}",
				)
