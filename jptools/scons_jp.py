...
def register_jp_builders(env: Any, dist_target: Any | None = None) -> None:
	...
	# jtalkSync output files (for dependency tracking)
	# These files are generated by jtalkSync and should trigger rebuild if modified
	repo_root = Path.cwd()
	jtalk_dir = repo_root / "source" / "synthDrivers" / "jtalk"
	jtalk_sync_outputs = []
	# sys.dic (dictionary file)
	sys_dic_path = jtalk_dir / "dic" / "sys.dic"
	if sys_dic_path.exists():
		jtalk_sync_outputs.append(env.File(str(sys_dic_path)))
	# libmecab.dll (copied by jtalkSync)
	libmecab_dll_path = jtalk_dir / "libmecab.dll"
	if libmecab_dll_path.exists():
		jtalk_sync_outputs.append(env.File(str(libmecab_dll_path)))
	# libopenjtalk.dll (copied by jtalkSync)
	libopenjtalk_dll_path = jtalk_dir / "libopenjtalk.dll"
	if libopenjtalk_dll_path.exists():
		jtalk_sync_outputs.append(env.File(str(libopenjtalk_dll_path)))
	...
	# Also depends on output files (if they exist) so that changes to these files trigger rebuild
	deps = [jtalk_prep_stamp]
	if jtalk_sync_outputs:
		deps.extend(jtalk_sync_outputs)
	env.Command(jtalk_sync_stamp, deps, _sync_jtalk_assets)
