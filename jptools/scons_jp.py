...
def register_jp_builders(env: Any, dist_target: Any | None = None, source_dir: Any | None = None) -> None:
	...
	def _sync_jtalk_assets(target: list[Any], source: list[Any], env: Any) -> int:
		# Use SCons top-level dir so repo_root is correct when cwd is a variant/build dir (e.g. CI)
		...
		def _dic_state(dic_dir: Path) -> tuple[bool, bool, bool]:
			...
			if not cp_file.exists():
				...
				try:
					...
					if cp_text in ("utf-8", "utf8"):
						has_valid_cp = True
					elif cp_text == "932":
						print(
							f"jtalkSync: DIC_CODEPAGE is 932 (legacy marker); rebuilding for UTF-8 make_jdic dictionary.",
						)
					else:
	...
	repo_root_clean = Path(env.Dir("#").abspath)
