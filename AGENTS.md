...
## スコープ（Step 1）
- 目的: 3.11 x86 を維持したまま「上流寄りのCI/ビルド基盤」に整合
- 除外: x64/arm64 切替、Java Access Bridge 64bit、Python 3.13 への移行
- 関連: Issue #539（Step 1）、Issue #530（上流 2026.1 マージ全体）

## 禁則と優先
- 破壊的操作を避ける（履歴の書換は明示指示時のみ）
- 上流との差分は最小化し、差分は明示的な場所に集約
...
- 型チェックのみの上流寄せワークフロー: `.github/workflows/nvbeta-typecheck-311x86.yml`
- 日本語版の包括的ワークフロー: `.github/workflows/testAndPublish.yml`

## 参照
- 人間向けハブ: `projectDocs/jp/README.md`
- 上流の開発環境: `projectDocs/dev/createDevEnvironment.md`
