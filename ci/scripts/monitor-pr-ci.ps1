            ...
            foreach ($fail in $failed) {
                Write-Host "`n[$($fail.name)]" -ForegroundColor Red
                    ...
                    if ($fail.name -like "*Build NVDA*" -or $fail.name -like "*Build*") {
                        ...
                        Write-Host "`n  [ADVICE] Test failures:" -ForegroundColor Cyan
                        ...
                        Write-Host "`n  [ADVICE] Type check failures:" -ForegroundColor Cyan
...
if ($Watch) {
    ...
    while ($true) {
        ...
        if ($currentState.Failed.Count -ne $lastState.Failed.Count) {
            $changed = $true
            Write-Host "`n[WARNING] Failure count changed!" -ForegroundColor Yellow
        }
    ...
    if ($status.Failed.Count -gt 0) {
        Write-Host "`n[FAIL] PR has $($status.Failed.Count) failing check(s)" -ForegroundColor Red
        exit 1
    } elseif ($status.InProgress.Count -gt 0) {
        Write-Host "`n[IN PROGRESS] PR has $($status.InProgress.Count) check(s) in progress" -ForegroundColor Yellow
        exit 0
    } else {
